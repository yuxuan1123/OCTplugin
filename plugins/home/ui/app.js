// home 插件页桥客户端 —— 与 demo_web/app.js 同构：
// 宿主 postMessage 传 {port,token,pluginId} → 本页直连内核 WS 鉴权 → 就近 RPC。
(() => {
  let ws = null; let seq = 0;
  const pending = new Map();
  const selfId = location.pathname.split("/")[2] || "home";
  ...
  // 插件自身方法调用：经 plugin.call 走 home 进程
  window.OCT = {
    pluginId: selfId,
    rpc,
    callHome: (method, params) => rpc("plugin.call", { pluginId: "home", method, params: params || {} })
      .then(r => r.result) // plugin.call 返回 {ok, result}，解包出真正的负载
  };
})();